    ...
    <div class="bg-white rounded-xl shadow p-6 max-w-3xl">
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500">
                Registra la información básica del técnico.
            </p>
        </div>

        <form method="POST" action="{{ route('admin.technicians.store') }}" class="space-y-6">
            @csrf
                ...
                <select name="speciality" class="w-full rounded-lg border-gray-300 text-sm">
                    ...
                    @foreach($services as $service)
                        <option value="{{ $service->name }}"
                            {{ old('speciality') == $service->name ? 'selected' : '' }}>
                            {{ $service->name }}
                        </option>
                    @endforeach
                </select>
                    ...
                    @foreach($days as $day)
                        <label class="flex items-center gap-2 text-sm border rounded-lg px-3 py-2">
                            <input type="checkbox"
                                ...
                                {{ in_array($day, $oldAvailability) ? 'checked' : '' }}>
                            {{ $day }}
                        </label>
                    @endforeach
                </div>
